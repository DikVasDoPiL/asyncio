...
from models import Base, Session, engine, swapi_people
    ...
    async with Session() as session:
        ...
        async with ClientSession() as session_deep:
            ...
            for person_json in people_chunk:
                ...
                homeworld_str = await get_deep_data(
                    [person_json["homeworld"]], "name", session_deep
                )

                films_str = await get_deep_data(
                    person_json["films"], "title", session_deep
                )

                species_str = await get_deep_data(
                    person_json["species"], "name", session_deep
                )

                starships_str = await get_deep_data(
                    person_json["starships"], "name", session_deep
                )

                vehicles_str = await get_deep_data(
                    person_json["vehicles"], "name", session_deep
                )

                newperson = swapi_people(
